...
class DepthLoss(nn.Module):
    ...
    def gradient_loss(self, pred, mask, gt):
        N = torch.sum(mask)
        diff = pred - gt
    ...
    def forward(self, pred, gt, mask):
        ...
        total_loss = self.w_data * self.data_loss(pred, mask, gt)
        total_loss += self.w_grad * self.gradient_loss(pred, mask, gt)
        total_loss += self.w_grad * self.gradient_loss(pred_1, mask_1, gt_1)
        total_loss += self.w_grad * self.gradient_loss(pred_2, mask_2, gt_2)
        total_loss += self.w_grad * self.gradient_loss(pred_3, mask_3, gt_3)
...
class DepthModel(_BaseModel):
    ...
    def test(self, input, name, WW, HH):
        ...
        minum = outputs.min(1).min(1)
        out_img = (outputs-minum[:, None, None])/(maximum[:, None, None] - minum[:, None, None])
